<?php 
  ...
  $id = $_SESSION['id']; /* Utilizando o id para associar aos JSON's, pois duas pessoas podem inserir o mesmo nome */
	...
	try {
	  ...
	  if ($busca > 0){
	  	$freq_json = json_decode(file_get_contents($linha['passos']));
	  }
	  ...
	  // Observar se os valores lidos no JSON já existem no Banco de Dados
		for ($i = 0; $i < 50; $i++){
		  ...
		  $value = $freq_json->feeds[$i]->field1;
			$query = "SELECT * FROM Passos WHERE created_at = '{$time}' AND id_paciente = {$id}";
		  $result = mysqli_query($conecta, $query);
		  $busca = mysqli_num_rows($result);
		  $linha = mysqli_fetch_assoc($result);
		  ...
		  if ($busca > 0){
		  	...
		  	$time = $freq_json->feeds[$i]->created_at;
		  	$value = $freq_json->feeds[$i]->field3;
		  	if ($value != null){
		  		$query = "INSERT INTO Passos (id_paciente, created_at, passos) VALUES ($id, '{$time}', {$value})";
		  		$result = mysqli_query($conecta, $query);
		  	}
		  }
		}
	...
	$query = "SELECT * FROM Passos WHERE id_paciente = $id";
	$result = mysqli_query($conecta, $query);
	$busca = mysqli_num_rows($result);
	...
	if ($busca > 0){
		while($row = mysqli_fetch_assoc($result)){
			...
			$jsonArrayItem['passos'] = $row['passos'];
